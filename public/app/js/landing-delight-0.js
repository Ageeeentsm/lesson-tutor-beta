...
(function(){
  ...
  function spawnMystery(){
    var key = 'ltMystery_'+ new Date().toISOString().slice(0,10);
    if (localStorage.getItem(key)) return; // already opened today
    var box = document.createElement('div'); box.id = 'lt-mystery';
    box.innerHTML = '🎁<span class="lt-mb-tip">Daily mystery box!</span>';
    document.body.appendChild(box);
    box.addEventListener('click', function(ev){
      if (box.classList.contains('opened')) return;
      box.classList.add('opened');
      box.innerHTML = '✨';
      var rewards = ['+10 XP & a Lucky Star ⭐','7-day streak shield 🛡️','Free Math Race entry 🧮','Surprise lesson unlocked 📘'];
      var pick = rewards[Math.floor(Math.random()*rewards.length)];
      var tip = document.createElement('span'); tip.className = 'lt-mb-tip'; tip.textContent = pick; box.appendChild(tip);
      spark(ev.clientX, ev.clientY, '✨');
      try { localStorage.setItem(key,'1'); } catch(e){}
      setTimeout(function(){ box.style.transition='opacity .6s'; box.style.opacity='0'; setTimeout(function(){box.remove();}, 700); }, 3500);
    });
  }
